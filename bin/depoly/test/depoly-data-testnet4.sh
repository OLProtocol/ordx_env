...
echo "deploying(reset) testnet4 ordx data on 192.168.10.102 for test"
...
echo "deploying(reset)  testnet4 ordx data on 192.168.10.101 for prd master"
ssh root@192.168.10.101 supervisorctl stop ordx-testnet4-master
ssh root@192.168.10.101 rm -rf /data/ordx-data/testnet4-master
ssh root@192.168.10.101 mkdir -p /data/ordx-data/testnet4-master
...
echo "deploying(reset) testnet4 ordx data on 192.168.10.101 for prd slave"
ssh root@192.168.10.101 supervisorctl stop ordx-testnet4-slave
ssh root@192.168.10.101 rm -rf /data/ordx-data/testnet4-slave
ssh root@192.168.10.101 mkdir -p /data/ordx-data/testnet4-slave
